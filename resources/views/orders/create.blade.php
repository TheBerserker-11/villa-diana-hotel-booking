...
<div class="container-xxl py-5">
    <div class="container" style="max-width: 980px;">
                        ...
                        <div class="p-3 border bg-white" style="border-radius:18px;">
                            ...
                            <div class="mt-3">
                                <div class="d-flex align-items-center justify-content-between">
                                    <h4 class="mb-0">{{ $room->roomtype->name ?? 'Room' }}</h4>
                                ...
                                <div class="text-muted mt-2">
                                    Includes <strong>{{ $room->included_pax ?? 1 }}</strong> pax -
                                    Max <strong>{{ $room->max_capacity ?? '-' }}</strong> pax
                                </div>
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                <div class="p-3 border bg-white" style="border-radius:16px;">
                                    ...
                                    <div class="fw-semibold">{{ $fields['check_in'] }}</div>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="p-3 border bg-white" style="border-radius:16px;">
                                    <div class="text-muted small">Check-out</div>
                                    <div class="fw-semibold">{{ $fields['check_out'] }}</div>
                                </div>
                            </div>
                            ...
                            <div class="col-6">
                                <div class="p-3 border bg-white" style="border-radius:16px;">
                                    ...
                                    <div class="fw-semibold">{{ $totalGuests }} total</div>
                            ...
                            <div class="col-6">
                                <div class="p-3 border bg-white" style="border-radius:16px;">
                                    ...
                                    <div class="fw-semibold">{{ $pricing['nights'] }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-4 border bg-white" style="border-radius:18px;">
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <strong>&#8369;{{ number_format($room->price ?? 0, 0) }}</strong>
                            </div>

                            @if(($pricing['extra_pax'] ?? 0) > 0)
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="text-muted">Extra pax fee (per night)</span>
                                    <strong>
                                        {{ $pricing['extra_pax'] }} x &#8369;{{ number_format(1200, 0) }}
                                    </strong>
                                </div>
                            @endif
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <strong id="computedPerNightAmount">&#8369;{{ number_format($pricing['per_night'], 0) }}</strong>
                            </div>
                            ...
                            <div class="mt-3 p-3 border rounded-3 bg-light-subtle vd-summary-extras">
                                ...
                                <div class="vd-extra-row">
                                    <div class="vd-extra-copy">
                                        ...
                                        <div class="vd-extra-price">(&#8369;{{ number_format($breakfastPrice, 0) }} each)</div>
                                    </div>
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">
                                    Room subtotal ({{ $pricing['nights'] }} night{{ $pricing['nights'] > 1 ? 's' : '' }})
                                </span>
                                <strong id="roomSubtotalAmount">&#8369;{{ number_format($roomSubTotal, 0) }}</strong>
                            </div>

                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Extras subtotal</span>
                                <strong id="breakfastTotalAmount">&#8369;{{ number_format($breakfastTotal, 0) }}</strong>
                            </div>

                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">Subtotal before VAT</span>
                                <strong id="subTotalAmount">&#8369;{{ number_format($subTotal, 0) }}</strong>
                            </div>

                            <div class="d-flex justify-content-between mb-2">
                                <span class="text-muted">VAT (10%)</span>
                                <strong id="vatAmount">&#8369;{{ number_format($vatAmount, 0) }}</strong>
                            </div>
        ...
        {{-- PAYMENT MODAL: only for logged-in users --}}
        @auth
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content" style="border-radius:18px; overflow:hidden;">
                    ...
                    <div class="modal-body text-center p-4">
                        ...
                        <img src="{{ asset('img/qrcodes/gcash.jpg') }}"
                             alt="GCash QR"
                             class="img-fluid"
                        ...
                        <form method="POST" action="{{ route('orders.store') }}" enctype="multipart/form-data">
                            ...
                            <input type="hidden" name="breakfast_qty" id="breakfastQtyInput" value="{{ $breakfastQty }}">

                            <input type="hidden" name="computed_per_night" value="{{ $pricing['per_night'] }}">
                            <input type="hidden" name="computed_nights" value="{{ $pricing['nights'] }}">
                            <input type="hidden" name="computed_total" id="computedTotalInput" value="{{ $grandTotal }}">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const nights = Number(@json((int) ($pricing['nights'] ?? 1)));
    const perNight = Number(@json((float) ($pricing['per_night'] ?? 0)));
