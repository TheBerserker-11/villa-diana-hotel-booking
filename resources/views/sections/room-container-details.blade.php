        ...
        <div class="text-center mb-5">
            ...
            @isset($totalGuests)
                <p class="text-muted mb-0" style="max-width:720px;margin:10px auto 0;">
                    Showing rooms that can accommodate <strong>{{ $totalGuests }}</strong>
                    guest{{ $totalGuests > 1 ? 's' : '' }} (closest capacity shown first).
                </p>
            @endisset
        </div>
                    ...
                    <div class="room-item shadow rounded overflow-hidden">
                        ...
                        <div class="position-relative">
                            ...
                            <small class="position-absolute start-0 top-0 bg-primary text-white rounded py-1 px-3 ms-3">
                                ₱{{ number_format((float)$perNight, 0) }}/Night
                            </small>
                        </div>
                        ...
                        <div class="p-4 mt-2">
                            ...
                            <h5 class="mb-1">{{ $room->roomtype->name ?? 'Room' }}</h5>

                            @if(!is_null($includedPax) && !is_null($maxCap))
                                <div class="text-muted small mb-1">
                                    Includes {{ $includedPax }} pax • Max {{ $maxCap }} pax
                                </div>
                            @endif

                            @if($extraPax > 0)
                                <div class="text-muted small mb-2">
                                    + {{ $extraPax }} extra pax × ₱{{ number_format(1200, 0) }} / night
                                </div>
                            @else
                                <div class="text-muted small mb-2">
                            ...
                            @if($incs->count())
                                <ul class="room-inclusions mb-2">
                                    ...
                                    @if($incs->count() > 3)
                                        <li class="text-muted small">+ {{ $incs->count() - 3 }} more</li>
                                    @endif
                                </ul>
                            @else
                                <ul class="room-inclusions mb-2">
                                    @foreach($defaultInclusions->take(3) as $inc)
                                        <li>
                                            <i class="bi bi-check-circle-fill text-success me-2"></i>
                                            {{ $inc }}
                                        </li>
                                    @endforeach
                                    <li class="text-muted small">+ {{ $defaultInclusions->count() - 3 }} more</li>
                                </ul>
                            @endif
                            ...
                            <div class="mt-3">
                                ...
                                <div class="d-flex justify-content-between">
                                    <a href="{{ route('room-tour', ['id' => $room->id] + request()->query()) }}"
                                    class="btn btn-sm btn-primary">
                                        View Room Tour
                                    </a>

                                    @if(($searched ?? false) === true && request()->filled('check_in') && request()->filled('check_out'))
                                        <a class="btn btn-sm btn-success"
                                        href="{{ route('booking.summary', request()->query() + ['room_id' => $room->id]) }}">
                                            Book Now
                                        </a>
                                    @else
